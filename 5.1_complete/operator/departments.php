...
switch ($page1) {
	...
	case 'edit':
		...
		if($jkp['email']!=''&&!filter_var($jkp['email'],FILTER_VALIDATE_EMAIL)){$errors['e1']=$jkl['e3'];}
		if(count($errors)==0){$result=$jakdb->update($jaktable,["title"=>$jkp['title'],"description"=>$jkp['description'],"email"=>$jkp['email'],"faq_url"=>$jkp['faq'],"time"=>$jakdb->raw("NOW()")],["id"=>$page2]);if(!$result){$_SESSION["infomsg"]=$jkl['i'];jak_redirect($_SESSION['LCRedirect']);}else{$cachestufffile=APP_PATH.JAK_CACHE_DIRECTORY.'/stuff.php';if(file_exists($cachestufffile)){unlink($cachestufffile);}
		JAK_base::jakWhatslog('',JAK_USERID,0,61,$page2,(isset($_COOKIE['WIOgeoData'])?$_COOKIE['WIOgeoData']:''),$jakuser->getVar("username"),$_SERVER['REQUEST_URI'],$ipa,$valid_agent);$_SESSION["successmsg"]=$jkl['g14'];jak_redirect($_SESSION['LCRedirect']);}}else{$errors=$errors;}}
		$SECTION_TITLE=$jkl["m17"];$SECTION_DESC="";require_once'../class/class.jaklic.php';$jakCheck=new JAKCheckAPI();if(!jak_get_important_data($ipa)){$jakCheck->deactivate_license();jak_redirect(JAK_rewrite::jakParseurl('maintenance'));}
		$JAK_FORM_DATA=jak_get_data($page2,$jaktable);$template='editdepartment.php';}else{$_SESSION["errormsg"]=$jkl['i3'];jak_redirect(JAK_rewrite::jakParseurl('departments'));}

    ...
    if (isset($jkp['save'])) {
        ...
        if (jak_get_access("settings", $jakuser->getVar("permissions"), JAK_SUPERADMINACCESS)) {
            ...
            if (count($errors) == 0) {
                ...
                $jakdb->update("settings", ["used_value" => $jkp['jak_chatgpt_version']], ["varname" => "chatgpt_version"]);
...
// Get importang settings data
if (!jak_get_important_data($ipa)) {
    // We could not receive the settings data, forward to
    $jakCheck->deactivate_license();

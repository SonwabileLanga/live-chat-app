...
switch ($page1) {
	...
	case 'edit':
		...
		default:
			...
			if($_SERVER['REQUEST_METHOD']=='POST'){if(isset($_POST['jak_delock_all'])){$jkp=$_POST;if(isset($jkp['action'])&&$jkp['action']=="lock"){$lockuser=$jkp['jak_delock_all'];$useridarray=explode(',',JAK_SUPERADMIN);for($i=0;$i<count($lockuser);$i++){$locked=$lockuser[$i];$uidacc=explode(":#:",$locked);if(!in_array($uidacc[0],$useridarray)){if($uidacc[1]==1){$query=$jakdb->update($jaktable,["access"=>0],["id"=>$uidacc[0]]);}else{$query=$jakdb->update($jaktable,["access"=>1],["id"=>$uidacc[0]]);}}}
			if($query){$_SESSION["successmsg"]=$jkl['g14'];jak_redirect($_SESSION['LCRedirect']);}
			$_SESSION["infomsg"]=$jkl['i1'];jak_redirect($_SESSION['LCRedirect']);}
			if(isset($jkp['delete'])&&$jkp['action']=="delete"){$lockuser=$jkp['jak_delock_all'];$useridarray=explode(',',JAK_SUPERADMIN);for($i=0;$i<count($lockuser);$i++){$locked=$lockuser[$i];$uidacc=explode(":#:",$locked);if(!in_array($uidacc[0],$useridarray)){$query=$jakdb->delete($jaktable,["id"=>$uidacc[0]]);}}
			if($query){$_SESSION["successmsg"]=$jkl['g14'];jak_redirect($_SESSION['LCRedirect']);}
			$_SESSION["infomsg"]=$jkl['i1'];jak_redirect($_SESSION['LCRedirect']);}}
			$_SESSION["infomsg"]=$jkl['i'];jak_redirect($_SESSION['LCRedirect']);}
			if(JAK_SUPERADMINACCESS){$JAK_USER_ALL=jak_get_user_all($jaktable,JAK_USERID,JAK_SUPERADMIN);}elseif(jak_get_access("usrmanage",$jakuser->getVar("permissions"),JAK_SUPERADMINACCESS)){$JAK_USER_ALL=jak_get_user_all($jaktable,JAK_USERID,JAK_SUPERADMIN);}else{$JAK_USER_ALL=jak_get_user_all($jaktable,JAK_USERID,'');}
			if($jakhs['hostactive']){$totalop=$jakdb->count($jaktable);if($totalop>=$jakhs['operators'])$newop=false;}
			$totalAll=$totalSeven=$total_voted=$total_logins=0;$total_vote='-';$totalAll=$jakdb->count($jaktable);$totalSeven=$jakdb->count($jaktable,["lastactivity[>=]"=>strtotime("-1 week")]);$total_logins=$jakdb->sum($jaktable,"logins");$total_voted=$jakdb->count("user_stats");$total_vote=$jakdb->sum("user_stats","vote");require_once'../class/class.jaklic.php';$jakCheck=new JAKCheckAPI();$verify_response=$jakCheck->verify_license(true);if($verify_response['status']!=true){if(JAK_SUPERADMINACCESS){jak_redirect(JAK_rewrite::jakParseurl('maintenance'));}else{$_SESSION["errormsg"]=$jkl['e27'];jak_redirect(BASE_URL);}}
			$SECTION_TITLE=$jkl["m4"];$SECTION_DESC="";$js_file_footer='js_user.php';$template='user.php';
}
?>
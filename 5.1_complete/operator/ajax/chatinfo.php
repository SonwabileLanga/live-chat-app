...
switch ($switchc) {
			...
			$clientinfo .= '<div class="media">
				    ...
				    <div class="media-body">
				        ...
				        <div class="media-text">
				        	<p class="text-muted"><i class="fa fa-clock"></i> '.JAK_base::jakTimesince($row['initiated'], JAK_DATEFORMAT, JAK_TIMEFORMAT).' | <i class="fa fa-eye"></i> '.$row['hits'].' | <i class="fa fa-language"></i> '.(!empty($row['lang']) ? strtoupper($row['lang']) : '-').'</p>
	...
	case 'edit':
		# code...
		$row = $jakdb->get("sessions", ["id", "operatorid", "department", "lang", "name", "email", "phone", "notes"], ["id" => $_POST['convid']]);
		...
		if (isset($row) && !empty($row)) {
			...
			// Get all departments
			$alldep = "";
			if (isset($LC_DEPARTMENTS) && is_array($LC_DEPARTMENTS)) foreach($LC_DEPARTMENTS as $z) {	
			...
			// Language
			$editclient .= '<div class="form-group">
			    <label for="name">'.$jkl["g22"].'</label>
			    <select name="jak_lang" id="language" class="form-control">
			    	'.$allang.'
				</select>
			</div>';
	...
	default:
		...
		if (isset($_POST['edit_customer']) && is_numeric($_POST['convid'])) {
			...
			if ($jkp['jak_depid'] != $jkp['olddepid']) {
				...
				$jakdb->update("sessions", ["department" => $jkp['jak_depid']], ["id" => $jkp['convid']]);

				// We will run the new name change process
				$namechange = true;
			}

			$result = $jakdb->update("sessions", ["lang" => $jkp['jak_lang'], "name" => $newname, "email" => filter_var($jkp['email'], FILTER_SANITIZE_EMAIL), "phone" => filter_var($jkp['phone'], FILTER_SANITIZE_NUMBER_INT), "notes" => filter_var($jkp['note'], FILTER_SANITIZE_FULL_SPECIAL_CHARS)], ["id" => $jkp['convid']]);
